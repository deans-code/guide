...
**Relationship to other patterns:** A module's public contract is a natural seam for extracting it into its own service later — at that point the in-process call becomes an HTTP/gRPC call, and the [Outbox Pattern](single-tech-projects.md) becomes relevant for keeping data in sync across the new service boundary.
